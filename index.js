...
console.log(
  connectStateMsgs.successMsg("PayPal configuration completed successfully.")
);
...
app.use(
  morgan(
    connectStateMsgs.requestMsg(
      "Request =>>>>",
      "Method - :method, URL -> :url, Status -> :status, Response -> :res[content-length] -> :response-time ms"
    )
  )
);
...
app.delete(
  ...
  CheckAuth.checkIsUser,
  CheckAuth.checkIsYourAccount,
  UserController.removeMyAccount
);
app.patch(
  "/auth/update_my_data/:id",
  CheckAuth.checkIsUser,
  CheckAuth.checkIsYourAccount,
  UserController.updateMyAccount
);
app.post(
  "/users/subscribe_to_posts",
  CheckAuth.checkIsUser,
  UserController.createSubscribe
);
app.get(
  "/users/check_subscribe_is_success",
  CheckAuth.checkIsUser,
  UserController.checkIsSuccess
);
//
...
app.listen(process.env.NODE_PORT, (error) => {
  if (error) {
    return console.log(
      connectStateMsgs.errorMsg("Error Server =>>>>>>>", error)
    );
  }
